<?php
/*
Plugin Name: Tag Cloud 3D
Plugin URI:
Description: 3D tag cloud displays your tags in 3D by placing them on a rotating text.
Version: 1.0
Author: scriptcase 
Author URI: http://www.scriptcase.net
License: GPL2
*/

add_action('admin_init', 'tagcloud_enq_scripts');
add_action("admin_menu", "scriptcase_3d_tag_cloud_options");
add_action('wp_enqueue_scripts', 'tagcloud_enq_scripts');
add_action("plugins_loaded", "scriptcase_3d_tagcloud_init");

function tagcloud_enq_scripts(){
	wp_enqueue_style('my-style', plugin_dir_url(__FILE__). '/public/css/scriptcase3dtagcloud.css');
	...
	if (isset($_GET['page']) && ($_GET['page'] == 'wp_3d_tag_cloud_slug'))
	{
		...
		if(is_admin()){
			...
			wp_enqueue_script('tag-3d-cloud-js', plugins_url('/public/js/scriptcase3dtagcloud.js', __FILE__), array('jquery'), '', true);
		}
	}
}
...
function scriptcase_3d_tag_cloud_options(){
	add_options_page(
		__('3D Tag Cloud'),
		__('3D Tag Cloud'),
		'manage_options',
		'wp_3d_tag_cloud_slug',
		'scriptcase_3D_tc_options_page');
}

function scriptcase_3D_tc_options_page(){
	require_once 'scriptcase_3D_tag_cloud_options.php';
}

function widget_scriptcase_3d_tagcloud($args){
	$option_value = retrieve_options();
	extract($args);
	echo $before_widget;
	echo $before_title;
...
function scriptcase_3d_tagcloud_init(){
	load_plugin_textdomain('scriptcasetagcloud', false, dirname( plugin_basename(__FILE__)).'/languages');
	wp_register_sidebar_widget('3d_tag_cloud', __('3D Tag Cloud'), 'widget_scriptcase_3d_tagcloud');
}
?>